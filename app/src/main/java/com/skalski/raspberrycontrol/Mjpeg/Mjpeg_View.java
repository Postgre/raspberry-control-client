...
public class Mjpeg_View extends SurfaceView implements SurfaceHolder.Callback {
	...
	private Mjpeg_InputStream mIn = null;
	...
	public class MjpegViewThread extends Thread {
		...
		private Rect destRect(int bmw, int bmh) {
			int tempx;
			int tempy;
			if (displayMode == Mjpeg_View.SIZE_STANDARD) {
				tempx = (dispWidth / 2) - (bmw / 2);
				tempy = (dispHeight / 2) - (bmh / 2);
				return new Rect(tempx, tempy, bmw + tempx, bmh + tempy);
			}
			if (displayMode == Mjpeg_View.SIZE_BEST_FIT) {
			...
			if (displayMode == Mjpeg_View.SIZE_FULLSCREEN)
	...
	private void init(Context context) {
		...
		if (!resume) {
			...
			ovlPos = Mjpeg_View.POSITION_LOWER_RIGHT;
			displayMode = Mjpeg_View.SIZE_STANDARD;
			dispWidth = getWidth();
	...
	public Mjpeg_View(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	public Mjpeg_View(Context context) {
		super(context);
		init(context);
	}
	...
	public void setSource(Mjpeg_InputStream source) {
		mIn = source;
		startPlayback();
	}
