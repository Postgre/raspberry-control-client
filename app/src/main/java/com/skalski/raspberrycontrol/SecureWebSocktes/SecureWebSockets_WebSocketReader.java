...
public class SecureWebSockets_WebSocketReader extends Thread {
	...
	private SecureWebSockets_NoCopyByteArrayOutputStream mMessagePayload;
	private ReaderState mState;
	private boolean mInsideMessage = false;
	private int mMessageOpcode;
	private SecureWebSockets_WebSocketFrameHeader mFrameHeader;
	private SecureWebSockets_Utf8Validator mUTF8Validator = new SecureWebSockets_Utf8Validator();
	 ...
	 */
	public SecureWebSockets_WebSocketReader(Handler master, Socket socket, SecureWebSockets_WebSocketOptions options, String threadName) {
		super(threadName);
		...
		this.mMessagePayload = new SecureWebSockets_NoCopyByteArrayOutputStream(options.getMaxMessagePayloadSize());
	...
	private boolean processData() throws Exception {
		...
		if (mFrameHeader == null) {
			...
			if (mApplicationBuffer.position() >= 2) {
				...
				if (rsv != 0) {
					throw new SecureWebSockets_WebSocketException("RSV != 0 and no extension negotiated");
				}

				if (masked) {
					// currently, we don't allow this. need to see whats the final spec.
					throw new SecureWebSockets_WebSocketException("masked server frame");
				}

				if (opcode > 7) {
					// control frame
					if (!fin) {
						throw new SecureWebSockets_WebSocketException("fragmented control frame");
					}
					if (payload_len1 > 125) {
						throw new SecureWebSockets_WebSocketException("control frame with payload length > 125 octets");
					}
					if (opcode != 8 && opcode != 9 && opcode != 10) {
						throw new SecureWebSockets_WebSocketException("control frame using reserved opcode " + opcode);
					}
					if (opcode == 8 && payload_len1 == 1) {
						throw new SecureWebSockets_WebSocketException("received close control frame with payload len 1");
					}
				} else {
					// message frame
					if (opcode != 0 && opcode != 1 && opcode != 2) {
						throw new SecureWebSockets_WebSocketException("data frame using reserved opcode " + opcode);
					}
					if (!mInsideMessage && opcode == 0) {
						throw new SecureWebSockets_WebSocketException("received continuation data frame outside fragmented message");
					}
					if (mInsideMessage && opcode != 0) {
						throw new SecureWebSockets_WebSocketException("received non-continuation data frame while inside fragmented message");
					}
				}
				...
				if (mApplicationBuffer.position() >= header_len) {
					...
					if (payload_len1 == 126) {
						...
						if (payload_len < 126) {
							throw new SecureWebSockets_WebSocketException("invalid data frame length (not using minimal length encoding)");
						}
						i += 2;
					} else if (payload_len1 == 127) {
						if ((0x80 & mApplicationBuffer.get(i+0)) != 0) {
							throw new SecureWebSockets_WebSocketException("invalid data frame length (> 2^63)");
						}
					...
					if (payload_len > mWebSocketOptions.getMaxFramePayloadSize()) {
						throw new SecureWebSockets_WebSocketException("frame payload too large");
					}

					// save frame header metadata
					mFrameHeader = new SecureWebSockets_WebSocketFrameHeader();
				...
				if (mFrameHeader.getOpcode() > 7) {
					...
					if (mFrameHeader.getOpcode() == 8) {
						...
						if (mFrameHeader.getPayloadLength() >= 2) {
							...
							if (code < 1000
								...
								throw new SecureWebSockets_WebSocketException("invalid close code " + code);
							}
							...
							if (mFrameHeader.getPayloadLength() > 2) {
								...
								SecureWebSockets_Utf8Validator val = new SecureWebSockets_Utf8Validator();
								val.validate(ra);
								if (!val.isValid()) {
									throw new SecureWebSockets_WebSocketException("invalid close reasons (not UTF-8)");
								} else {
									reason = new String(ra, SecureWebSockets_WebSocket.UTF8_ENCODING);
								}
							}
						}
						onClose(code, reason);
					...
					if (framePayload != null) {
						...
						if (mMessagePayload.size() + framePayload.length > mWebSocketOptions.getMaxMessagePayloadSize()) {
							throw new SecureWebSockets_WebSocketException("message payload too large");
						}
						...
						if (mMessageOpcode == 1 && mWebSocketOptions.getValidateIncomingUtf8() && !mUTF8Validator.validate(framePayload)) {
							throw new SecureWebSockets_WebSocketException("invalid UTF-8 in text message payload");
						}
					...
					if (mFrameHeader.isFin()) {
						...
						if (mMessageOpcode == 1) {
							...
							if (mWebSocketOptions.getValidateIncomingUtf8() && !mUTF8Validator.isValid()) {
								throw new SecureWebSockets_WebSocketException("UTF-8 text message payload ended within Unicode code point");
							}
							...
							if (mWebSocketOptions.getReceiveTextMessagesRaw()) {
								...
								String s = new String(mMessagePayload.toByteArray(), SecureWebSockets_WebSocket.UTF8_ENCODING);
								onTextMessage(s);
							}
	...
	protected void onHandshake(boolean success) {
		notify(new SecureWebSockets_WebSocketMessage.ServerHandshake(success));
	}
	...
	protected void onClose(int code, String reason) {
		notify(new SecureWebSockets_WebSocketMessage.Close(code, reason));
	}
	...
	protected void onPing(byte[] payload) {
		notify(new SecureWebSockets_WebSocketMessage.Ping(payload));
	}
	...
	protected void onPong(byte[] payload) {
		notify(new SecureWebSockets_WebSocketMessage.Pong(payload));
	}
	...
	protected void onTextMessage(String payload) {
		notify(new SecureWebSockets_WebSocketMessage.TextMessage(payload));
	}
	...
	protected void onRawTextMessage(byte[] payload) {
		notify(new SecureWebSockets_WebSocketMessage.RawTextMessage(payload));
	}
	...
	protected void onBinaryMessage(byte[] payload) {
		notify(new SecureWebSockets_WebSocketMessage.BinaryMessage(payload));
	}
			...
			if (mApplicationBuffer.get(pos+0) == 0x0d &&
				...
				if (mApplicationBuffer.get(0) == 'H' &&
					...
					if (status.first >= 300) {
						...
						notify(new SecureWebSockets_WebSocketMessage.ServerError(status.first, status.second));
						serverError = true;
					}
				}
	...
	private Pair<Integer, String> parseHTTPStatus() throws UnsupportedEncodingException {
		...
		String statusMessage = new String(statusBuf, SecureWebSockets_WebSocket.UTF8_ENCODING);
	...
	public void run() {		
		...
		while (!mStopped) {
			try {
				...
				notify(new SecureWebSockets_WebSocketMessage.ProtocolViolation(e));
