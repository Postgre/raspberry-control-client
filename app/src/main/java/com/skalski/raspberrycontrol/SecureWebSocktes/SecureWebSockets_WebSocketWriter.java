...
public class SecureWebSockets_WebSocketWriter extends Thread {
	...
	private final SecureWebSockets_WebSocketOptions mWebSocketOptions;
	private final ByteBuffer mApplicationBuffer;
	private final Socket mSocket;
	private OutputStream mOutputStream;
	private Handler mHandler;

	public SecureWebSockets_WebSocketWriter(Handler master, Socket socket, SecureWebSockets_WebSocketOptions options, String threadName) {
		super(threadName);
	 ...
	 */
	private void sendClientHandshake(SecureWebSockets_WebSocketMessage.ClientHandshake message) throws IOException {
	 ...
	 */
	private void sendClose(SecureWebSockets_WebSocketMessage.Close message) throws IOException, SecureWebSockets_WebSocketException {
		if (message.getCode() > 0) {
			byte[] payload = null;

			if (message.getReason() != null && !(message.getReason().length() > 0)) {
				byte[] pReason = message.getReason().getBytes(SecureWebSockets_WebSocket.UTF8_ENCODING);
			...
			if (payload != null && payload.length > 125) {
				throw new SecureWebSockets_WebSocketException("close payload exceeds 125 octets");
			}
	 ...
	 */
	private void sendPing(SecureWebSockets_WebSocketMessage.Ping message) throws IOException, SecureWebSockets_WebSocketException {
		if (message.mPayload != null && message.mPayload.length > 125) {
			throw new SecureWebSockets_WebSocketException("ping payload exceeds 125 octets");
		}
	 ...
	 */
	private void sendPong(SecureWebSockets_WebSocketMessage.Pong message) throws IOException, SecureWebSockets_WebSocketException {
		if (message.mPayload != null && message.mPayload.length > 125) {
			throw new SecureWebSockets_WebSocketException("pong payload exceeds 125 octets");
		}
	 ...
	 */
	private void sendBinaryMessage(SecureWebSockets_WebSocketMessage.BinaryMessage message) throws IOException, SecureWebSockets_WebSocketException {
		if (message.mPayload.length > mWebSocketOptions.getMaxMessagePayloadSize()) {
			throw new SecureWebSockets_WebSocketException("message payload exceeds payload limit");
		}
	 ...
	 */
	private void sendTextMessage(SecureWebSockets_WebSocketMessage.TextMessage message) throws IOException, SecureWebSockets_WebSocketException {
		byte[] payload = message.mPayload.getBytes(SecureWebSockets_WebSocket.UTF8_ENCODING);
	 ...
	 * Send WebSockets binary message.
	 */
	private void sendRawTextMessage(SecureWebSockets_WebSocketMessage.RawTextMessage message) throws IOException, SecureWebSockets_WebSocketException {
		if (message.mPayload.length > mWebSocketOptions.getMaxMessagePayloadSize()) {
			throw new SecureWebSockets_WebSocketException("message payload exceeds payload limit");
		}
	 ...
	 */
	protected void processMessage(Object msg) throws IOException, SecureWebSockets_WebSocketException {

		if (msg instanceof SecureWebSockets_WebSocketMessage.TextMessage) {
			sendTextMessage((SecureWebSockets_WebSocketMessage.TextMessage) msg);
		} else if (msg instanceof SecureWebSockets_WebSocketMessage.RawTextMessage) {
			sendRawTextMessage((SecureWebSockets_WebSocketMessage.RawTextMessage) msg);
		} else if (msg instanceof SecureWebSockets_WebSocketMessage.BinaryMessage) {
			sendBinaryMessage((SecureWebSockets_WebSocketMessage.BinaryMessage) msg);
		} else if (msg instanceof SecureWebSockets_WebSocketMessage.Ping) {
			sendPing((SecureWebSockets_WebSocketMessage.Ping) msg);
		} else if (msg instanceof SecureWebSockets_WebSocketMessage.Pong) {
			sendPong((SecureWebSockets_WebSocketMessage.Pong) msg);
		} else if (msg instanceof SecureWebSockets_WebSocketMessage.Close) {
			sendClose((SecureWebSockets_WebSocketMessage.Close) msg);
		} else if (msg instanceof SecureWebSockets_WebSocketMessage.ClientHandshake) {
			sendClientHandshake((SecureWebSockets_WebSocketMessage.ClientHandshake) msg);
		} else if (msg instanceof SecureWebSockets_WebSocketMessage.Quit) {
	...
	public void writeMessageToBuffer(Message message) {
		try {
			...
			notify(new SecureWebSockets_WebSocketMessage.Error(e));
		}
	}
	 ...
	 */
	protected void processAppMessage(Object msg) throws SecureWebSockets_WebSocketException, IOException {
		throw new SecureWebSockets_WebSocketException("unknown message received by WebSocketWriter");
	}
	...
	private static class ThreadHandler extends Handler {
		private final WeakReference<SecureWebSockets_WebSocketWriter> mWebSocketWriterReference;

		public ThreadHandler(SecureWebSockets_WebSocketWriter webSocketWriter) {
			super();
			this.mWebSocketWriterReference = new WeakReference<SecureWebSockets_WebSocketWriter>(webSocketWriter);
		}

		@Override
		public void handleMessage(Message message) {
			SecureWebSockets_WebSocketWriter webSocketWriter = mWebSocketWriterReference.get();
